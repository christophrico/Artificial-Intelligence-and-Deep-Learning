...
def discriminator_model():
    model = Sequential()
    model.add(
        Conv2D(64, (5, 5), padding="same", input_shape=(28, 28, 1), name="discrim_64")
    )
    model.add(Activation("tanh"))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Conv2D(128, (5, 5), name="Discrim_128"))
    model.add(Activation("tanh"))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Flatten())
...
def train(BATCH_SIZE, EPOCHS, WARM_START=False):
    ...
    for epoch in range(epochs_start, EPOCHS):
        ...
        for index in range(int(X_train.shape[0] / BATCH_SIZE)):
            ...
            # print stats
            print(
                "Batch {}   d_loss: {:.2f}    g_loss: {:.2f}".format(
                    index, d_loss, g_loss
                )
            )
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        train(
            BATCH_SIZE=args.batch_size, EPOCHS=args.epochs, WARM_START=args.warm_start
        )
